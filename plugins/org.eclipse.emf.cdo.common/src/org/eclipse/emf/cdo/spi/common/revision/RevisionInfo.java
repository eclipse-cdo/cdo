...
public abstract class RevisionInfo
{
  ...
  public void writeResult(CDODataOutput out, int referenceChunk) throws IOException
  {
    ...
    writeResult(out, referenceChunk, synthetic);
  }
   ...
   * @since 3.0
   */
  public static void writeResult(CDODataOutput out, int referenceChunk, InternalCDORevision revision)
      throws IOException
  {
    if (revision == null)
    {
    ...
    else if (revision instanceof PointerCDORevision)
    {
      ...
      {
        writeResult(out, referenceChunk, (InternalCDORevision)target);
      }
      else
      {
        out.writeByte(NO_RESULT);
  ...
  {
    writeResult(out, referenceChunk, revision);
  }
